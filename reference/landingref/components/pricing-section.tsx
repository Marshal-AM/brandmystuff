...
import { GradeBadge, SectionHeading } from "./section-kit"
...
function SpaceCard({ l }: { l: Listing }) {
  return (
...
export function PricingSection() {
  ...
  return (
    <section id="marketplace" className="overflow-hidden bg-p-50 py-32">
      <div className="mx-auto mb-16 max-w-7xl px-6">
        <SectionHeading
          kicker="Marketplace"
          title="Real spaces on real things, ranked by quality"
          sub="Every listing is a single ad space with its own close-up photo, score, size and fixed weekly price. Filter by city, category, grade or budget, then lease in one click."
        />
      </div>

      <div className="relative w-full" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-24 bg-gradient-to-r from-p-50 to-transparent" />
        <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-24 bg-gradient-to-l from-p-50 to-transparent" />
