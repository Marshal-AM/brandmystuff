...
import { BASE, fx, step, sleep, rand, actor, http, api, form, file, signIn, runTx, ev, fund, sweep, logoPng, proofPhoto } from "./helpers";
  ...
  try {
    ...
    await sql.begin(async (tx) => {
      ...
      if (spaces.length) {
        ...
        if (offs.length) {
          await tx`delete from payouts where offering_id in ${tx(offs)}`;
